...
namespace btree
{
	...
	struct Node
	{
		int getRightDepth(int i = 0);
		int getLeftDepth(int i = 0);
		...
		void scanUp(void(*f)(void* n)); // обход дерева снизу вверх
		void scanMid(void(*f)(void* n)); // обход слева направо
	...
	{
		int zad(Node* tree, bool(is2)(void* n));
		bool isBalansed(Node* element);
