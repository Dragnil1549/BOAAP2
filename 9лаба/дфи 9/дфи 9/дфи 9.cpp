...
Tree* makeTree(Tree* Root) //Создание дерева 
{
	...
	if (Root == NULL) // если дерево не создано 
	{
		...
		cin >> key; cout << "Введите слово корня: ";
		cin >> s;
...
Tree* delet(Tree* Root, int key) //Удаление элемента по ключу
{
	...
	else // поддерево R присоединяется к родителю удаляемого узла 
...
int zad(Tree* tree) { // задание (вариант 12) -- посчитать узлы с чётными ключами
	if (!tree) {
		return 0;
	}
...
int dop1(Tree* tree) { // доп 1 (вариант 10) -- найти сумму всех вершин дерева
	if (!tree) {
		return 0;
	}
...
int dop2(Tree* tree) { // доп 2 (вариант 2) -- посчитать количество листьев, являющихся правыми дочерними вершинами
	if (!tree) {
		return 0;
	}
...
int dop3(Tree* tree) { //доп 3 (вариант 5) -- посчитать количество левых дочерних вершин
	if (!tree) {
		return 0;
	}
