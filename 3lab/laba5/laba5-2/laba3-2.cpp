...
int inputSitizen(int size) {
	...
	cout << "Место работы/учёбы:" << endl;
	cout << "0 - работа, 1 - учёба" << endl;
	cin >> list[current].p;
	if (list[current].p) {
		cin >> list[current].plac.uch;
	}
	else {
		cin >> list[current].plac.rab;
	}
...
int showSitizen(int index) { //вспомогательная функция, выводит на экран одну запись
	...
	if (list[index].p) {
		cout << "uch " << list[index].plac.uch;
	}
	else {
		cout << "rab " << list[index].plac.rab;
	}
