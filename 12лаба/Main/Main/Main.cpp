#include "Hash.h"
#include <tchar.h>
#include <iostream>
using namespace std;
struct AAA
{
    int key;
    const char* mas;
    AAA(int k, const char* z)
    {
        key = k;  mas = z;
    } AAA() {}
};
//-------------------------------
int key(void* d)
{
    AAA* f = (AAA*)d;   return f->key;
}
//-------------------------------
void AAA_print(void* d)
{
    cout << " ключ " << ((AAA*)d)->key << " - " << ((AAA*)d)->mas << endl;
}
//-------------------------------
int _tmain(int argc, _TCHAR* argv[])
{
    setlocale(LC_ALL, "rus");
    AAA a1(1, "one"), a2(2, "two"), a3(4, "three"), a4(2, "four");
    int siz, choice, k;
    cout << "Введите размер хеш-таблицы" << endl;   cin >> siz;
    Object H = create(siz, key);
    for (;;)
    {
        cout << "1 - вывод хеш-таблицы" << endl;
        cout << "2 - добавление элемента" << endl;
        cout << "3 - удаление элемента" << endl;
        cout << "4 - поиск элемента" << endl;
        cout << "5 - доступ к последовательным элементам в несортированном порядке" << endl;
        cout << "0 - выход" << endl;
        cout << "сделайте выбор" << endl;   cin >> choice;
        switch (choice)
        {
        case 0:  exit(0);
        case 1: H.scan(AAA_print);  break;
        case 2: { AAA* a = new AAA;
            char* str = new char[20];
            cout << "введите ключ" << endl;  cin >> k;
            a->key = k;
            cout << "введите строку" << endl; cin >> str;
            a->mas = str;
            /* if (H.N == H.size)
                 cout << "Таблица заполнена" << endl;*/
            if (H.N >= H.size / 2) {  //Моя основа
                H.insert(a);
                cout << "Таблица заполнена на 50%, увеличиваем размер вдвое" << endl;
                Object Temp = H;
                siz *= 2;
                H = create(siz, key);
                for (int i = 0; i < Temp.size; i++) {
                    if (Temp.data[i]) {
                        H.insert(Temp.data[i]);
                    }
                }
            }
            //if (H.N >= H.size * 0.7) {  //доп вариант 16
            //    H.insert(a);
            //    cout << "Таблица заполнена на 70%, увеличиваем размер втрое" << endl;
            //    Object Temp = H;
            //    siz *= 3;
            //    H = create(siz, key);
            //    for (int i = 0; i < Temp.size; i++) {
            //        if (Temp.data[i]) {
            //            H.insert(Temp.data[i]);
            //        }
            //    }
            //    H.N = Temp.N;
            //}
            else
                H.insert(a);
        } break;
        case 3: {  cout << "введите ключ для удаления" << endl;
            cin >> k;
            H.deleteByKey(k);
        }  break;
        case 4: {  cout << "введите ключ для поиска" << endl;
            cin >> k;
            if (H.search(k) == NULL)
                cout << "Элемент не найден" << endl;
            else
                AAA_print(H.search(k));
        }  break;
        case 5: H.showElements(AAA_print); break; //доп вариант 11
        }
    }
    return 0;
}
